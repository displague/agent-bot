...
def generate_llama_response(prompt, assistant_private_notes):
    global llm_context
...
User: {prompt}
Assistant:"""
            response = llm(internal_prompt, max_tokens=150)
...
def generate_assistant_private_notes(prompt):
...
Assistant's Private Notes:"""
            response = llm(analysis_prompt, max_tokens=100)
            assistant_private_notes = response['choices'][0]['text'].strip()
...
def process_private_notes(private_notes, from_assistant=False):
    ...
    if "I should look for previous conversations about" in private_notes:
        ...
        asyncio.run_coroutine_threadsafe(schedule_event(event), asyncio.get_event_loop())
...
Summary:"""
        ...
        asyncio.run_coroutine_threadsafe(schedule_event(rag_event), asyncio.get_event_loop())
    except Exception as e:
...
async def render_tui(stdscr):
    ...
    while True:
        ...
        if key == 27:  # ESC key to switch screens
            ...
            curses.echo()
            private_notes = stdscr.getstr(max_y - 1, len("Enter Private Notes: ")).decode()
            curses.noecho()
            ...
            interaction_queue.put({
                "user_input": input_buffer,
                "user_private_notes": private_notes
            })
...
async def process_interactions():
    ...
    while True:
        try:
            ...
            assistant_private_notes = generate_assistant_private_notes(user_input)
            ...
            response = generate_llama_response(user_input, assistant_private_notes)
            logger.info(f"Response: {response}")
            interaction_log.append(f"Assistant: {response}")
            # Log interaction
            log_entry = {
                "timestamp": datetime.now().isoformat(),
